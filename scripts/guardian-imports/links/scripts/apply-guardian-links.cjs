...
async function main() {
  ...
  printResults(isApply ? "GuardianLink apply plan" : `GuardianLink dry run (pass --apply=${APPLY_TOKEN} to write)`, results);
  if (!isApply) {
    if (results.some((result) => result.action === "BLOCKED")) process.exitCode = 1;
    return;
  }
  const applied = [];
  for (const result of results) applied.push(await applyGuardianLinkResult({ db, orgId: config.orgId, result }));
  console.table(applied);
  if (applied.some((result) => result.action === "BLOCKED")) process.exitCode = 1;
}

main().catch((error) => {
  console.error("GuardianLink apply failed:", error);
  process.exitCode = 1;
});
