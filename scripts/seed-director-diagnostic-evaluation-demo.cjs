...
const FRAMEWORK_ID = "director-diagnostic-teacher-evaluation-v1";
...
async function main() {
  ...
  console.log("Seeding director diagnostic evaluation demo...");
  console.log({
    orgId: ORG_ID,
    schoolId: SCHOOL_ID,
  ...
  const plan = {
    ...
    title: "التقييم التشخيصي لمعلمي منار الريادة بنين السيح - الفصل الأول",
    description:
      "خطة تجريبية لتطبيق القالب الرسمي للتقييم التشخيصي للمعلمين مرتين داخل الفصل الدراسي.",
  ...
  const cycles = [
    {
      number: 1,
      id: `${PLAN_ID}-diagnostic-01`,
      title: "التقييم التشخيصي الأول",
    },
    {
      number: 2,
      id: `${PLAN_ID}-diagnostic-02`,
      title: "التقييم التشخيصي الثاني",
    },
  ];
  ...
  for (const cycleSeed of cycles) {
    ...
    const evaluatorAssignmentId = `${PLAN_ID}-${cycleSeed.id}-${targetPerson.id}-${evaluatorPerson.id}`;
    ...
    writes.push(upsert(db, `orgs/${ORG_ID}/evaluationCycles/${cycle.id}`, cycle));
  ...
  console.log("\n✅ Director diagnostic evaluation demo seed completed.");
  console.log({
    planId: PLAN_ID,
    cyclesCreatedOrUpdated: cycles.length,
    targetPersonId: targetPerson.id,
    evaluatorPersonId: evaluatorPerson.id,
  });
}
...
main().catch((error) => {
  console.error("\n❌ Director diagnostic evaluation demo seed failed:");
  console.error(error);
  process.exit(1);
});