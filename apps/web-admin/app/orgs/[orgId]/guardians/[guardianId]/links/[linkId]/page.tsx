...
export default function EditGuardianLinkPage() {
  const router = useRouter();
  const params = useParams<{ orgId: string; guardianId: string; linkId: string }>();
  ...
  const loadSummary = useCallback(async (): Promise<PageData | null> => {
    ...
    const [guardianSnap, linkSnap, studentsSnap, peopleSnap] = await Promise.all([
      getDoc(guardianRef),
      getDoc(linkRef),
    ...
    if (!guardianSnap.exists() || !linkSnap.exists()) {
      return null;
    }
  ...
  }, [orgId, guardianId, linkId]);

  const { data, loading, error, notFound } = useDocumentLoader<PageData>({
    enabled: !!user,
    loader: loadSummary,
    deps: [orgId, guardianId, linkId],
  });

  useEffect(() => {
    if (!data) return;

    setStudentId(data.link.studentId);
  ...
  if (notFound) {
    return (
      <PageHero
        ...
        description="قد يكون الرابط غير موجود أو لا يتبع ولي الأمر الحالي."
        actions={
  ...
  return (
    <div className="space-y-6">
      ...
      <FormSection
        title="بيانات الرابط"
        description="عدّل الطالب ونوع العلاقة وحالة الرابط."
