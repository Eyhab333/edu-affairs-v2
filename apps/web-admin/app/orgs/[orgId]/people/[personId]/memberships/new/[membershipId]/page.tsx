...
export default function EditMembershipPage() {
  ...
  const loadPageData = useCallback(async (): Promise<PageData | null> => {
    ...
    const [personSnap, membershipSnap, schoolsSnap, peopleSnap] =
      await Promise.all([
        getDoc(personRef),
    ...
    const membership = membershipSnap.data() as MembershipRow;
  ...
  useEffect(() => {
    ...
    setRoleKey(String(data.membership.roleKey || data.membership.role || ""));
    setTitle(data.membership.title || "");
    setDepartment(data.membership.department || "");
    setScopeType(String(data.membership.scopeType || "ORG"));
    setScopeId(data.membership.scopeId || "");
    setStartAt(toDateInputValue(data.membership.startAt));
    setEndAt(toDateInputValue(data.membership.endAt));
    setIsActive(data.membership.isActive !== false);

    setDirectEvaluatorPersonId(data.membership.directEvaluatorPersonId || "");
    setSupervisorPersonId(data.membership.supervisorPersonId || "");
    setManagerPersonId(data.membership.managerPersonId || "");
    setPrincipalPersonId(data.membership.principalPersonId || "");
    setVicePrincipalPersonId(data.membership.vicePrincipalPersonId || "");

    setCreatedAt(data.membership.createdAt);
  }, [data]);
  ...
  return (
    <div className="space-y-6">
      ...
      <FormSection
        ...
        description="عدّل الدور والنطاق وحالة التفعيل."
      ...
      <FormSection
        ...
        description="هذه الحقول تجعل bootstrap والتوزيع التلقائي أدق كثيرًا."
        contentClassName="space-y-4"
      >
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2">
