...
type PageData = {
  ...
  assignment: {
    ...
    targetScopeType: AssignmentScopeValue;
    targetScopeId: string;
    coverageMode: CoverageModeValue;
    subjectKey?: string;
    subjectId?: string;
    gradeId?: string;
    streamId?: string;
    isHomeroom?: boolean;
    roleInAssignment?: RoleInAssignmentValue;
    status?: AssignmentStatusValue;
    note?: string;
    startAt?: number;
    endAt?: number;
  };
  assignmentClassLinks: {
    id: string;
    assignmentId: string;
    classId: string;
  }[];
...
export default function EditAssignmentPage() {
  ...
  const loadPageData = useCallback(async (): Promise<PageData | null> => {
    ...
    return {
      schoolName: (schoolSnap.data() as { name?: string }).name ?? "المدرسة",
      schoolType: SchoolType.safeParse(
        (schoolSnap.data() as { profile?: { schoolType?: SchoolTypeValue } })
          .profile?.schoolType,
      ).success
        ? ((schoolSnap.data() as { profile?: { schoolType?: SchoolTypeValue } })
            .profile?.schoolType as SchoolTypeValue)
        : "PRIMARY",
      yearTitle:
      ...
      assignment: {
        ...
        ...(assignmentSnap.data() as Omit<PageData["assignment"], "id">),
      },
      assignmentClassLinks: linksSnap.docs
        .map((item) => ({
          id: item.id,
          ...(item.data() as Omit<
            PageData["assignmentClassLinks"][number],
            "id"
          >),
        }))
        .filter((item) => item.assignmentId === assignmentId),
    };
  }, [orgId, schoolId, yearId, assignmentId]);

  const { data, loading, error, notFound } = useDocumentLoader<PageData>({
    enabled: !!user,
    loader: loadPageData,
    deps: [orgId, schoolId, yearId, assignmentId],
  });

  useEffect(() => {
    if (!data) return;

    setTeacherPersonId(data.assignment.teacherPersonId);
    setSupervisorPersonId(data.assignment.supervisorPersonId ?? "");
    setAssignmentKind(data.assignment.assignmentKind);
    setTargetScopeType(data.assignment.targetScopeType);
    setTargetScopeId(data.assignment.targetScopeId);
    setCoverageMode(data.assignment.coverageMode);
    setSubjectId(data.assignment.subjectId ?? "");
    setGradeId(data.assignment.gradeId ?? "");
    setStreamId(data.assignment.streamId ?? "");
    setIsHomeroom(Boolean(data.assignment.isHomeroom));
    setRoleInAssignment(
      (data.assignment.roleInAssignment as RoleInAssignmentValue) ?? "MAIN",
    );
    setStatus((data.assignment.status as AssignmentStatusValue) ?? "ACTIVE");
    setNote(data.assignment.note ?? "");
    setSelectedClassIds(data.assignmentClassLinks.map((item) => item.classId));
  }, [data]);
  ...
  async function save() {
    ...
    try {
      ...
      const payload = {
        ...
        roleInAssignment,
        status,
        startAt: data?.assignment.startAt ?? nowMs,
        endAt: data?.assignment.endAt,
        note: note.trim(),
        updatedAt: nowMs,
      };
      ...
      if (coverageMode === "EXPLICIT_CLASSES") {
        for (const classId of selectedClassIds) {
          ...
          await setDoc(linkRef, parsedLink.data, { merge: true });
        }

        for (const existing of data?.assignmentClassLinks ?? []) {
          if (!nextLinkIds.has(existing.classId)) {
            const linkRef = doc(
              db,
              `orgs/${orgId}/schools/${schoolId}/academicYears/${yearId}/teacherAssignmentClassLinks/${existing.id}`,
            );
      ...
      toast.success("تم حفظ الإسناد التعليمي بنجاح");
      router.push(
      ...
      toast.error("تعذر حفظ الإسناد التعليمي");
    } finally {
      setSaving(false);
    }
  }
  ...
  if (notFound) {
    return (
      <PageHero
        ...
        description="قد تكون المدرسة أو السنة أو الإسناد غير موجود."
        actions={
  ...
  return (
    <div className="space-y-6">
      ...
      <FormSection
        ...
        description="عدّل بيانات الإسناد وربطه بالفصول حسب الحاجة."
