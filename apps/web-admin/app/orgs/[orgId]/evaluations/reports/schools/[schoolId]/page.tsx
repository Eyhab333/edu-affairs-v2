...
export default function EvaluationSchoolReportPage() {
  ...
  const loadPage = useCallback(async (): Promise<PageData | null> => {
    ...
    const [schoolSnap, plansSnap, cyclesSnap, peopleSnap, submissionsSnap] =
      await Promise.all([
        getDoc(schoolRef),
  ...
  useEffect(() => {
    if (error) toast.error("تعذر تحميل لوحة المدرسة");
  }, [error]);
  ...
  }, [data?.submissions, planId, cycleId, targetRoleKey]);

  const totalSubmissions = filteredRows.length;
  const uniqueTargets = new Set(
    filteredRows.map((row) => getSubmissionTargetPersonId(row)).filter(Boolean)
  ).size;
  const approvedCount = filteredRows.filter((row) =>
    ["APPROVED", "LOCKED"].includes(row.status)
  ).length;
  const averagePercentage =
    filteredRows.length > 0
      ? round2(
          filteredRows.reduce((sum, row) => sum + getPercentage(row), 0) /
            filteredRows.length
        )
      : 0;
  ...
  const latestRows = useMemo(() => filteredRows.slice(0, 10), [filteredRows]);
  ...
  if (notFound) {
    return (
      <PageHero
        ...
        title="تعذر العثور على المدرسة"
        description="قد لا تكون هذه المدرسة موجودة."
        actions={
  ...
  return (
    <div className="space-y-6">
      <PageHero
        badge="لوحة المدرسة"
        badgeIcon={<Building2 className="h-3.5 w-3.5" />}
        title={data?.school.name ?? "المدرسة"}
        description="ملخص تقييمات المدرسة مع مقارنات حسب الخطة والدورة والدور المستهدف."
        actions={
          <Button asChild variant="outline">
            <Link href={`/orgs/${orgId}/evaluations/reports`}>
              <ArrowLeft className="h-4 w-4" />
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <InfoCard label="الإدخالات" value={totalSubmissions} hint="بعد الفلاتر" />
        ...
        <InfoCard label="متوسط النسبة %" value={averagePercentage} hint="Average %" />
      </div>
      ...
      <FormSection
        title="الفلاتر"
        description="فلترة داخل هذه المدرسة فقط."
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <FormSection
          title="حسب الخطة"
          description="مقارنة داخل المدرسة حسب الخطة."
          contentClassName="space-y-4"
        >
          {byPlan.length === 0 ? (
        ...
        <FormSection
          title="حسب الدورة"
          description="مقارنة داخل المدرسة حسب الدورة."
          contentClassName="space-y-4"
        >
          {byCycle.length === 0 ? (
            <div className="rounded-2xl border border-dashed px-6 py-10 text-center text-sm text-muted-foreground">
      ...
      <FormSection
        title="حسب الدور المستهدف"
        description="مقارنة حسب targetRole داخل المدرسة."
        contentClassName="space-y-4"
      >
      ...
      <FormSection
        ...
        description="أحدث إدخالات هذه المدرسة."
        contentClassName="space-y-4"
      >
        {latestRows.length === 0 ? (
